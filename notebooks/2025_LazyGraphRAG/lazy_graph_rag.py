...
def create_graph(documents: Iterable[Document],
                 *,
                directed: bool = False,
                edges: Iterable[str | Tuple[str, str]]) -> Tuple[dict[str, Document], nx.Graph]:
    ...
    for document in documents:
        ...
        for to_field in to_fields:
            for to_value in _get_md_values(document.metadata, to_field):
                documents_by_to_field.setdefault((to_field, to_value), set()).add(document.id)
    ...
    return documents_by_id, graph

def group_by_community(documents_by_id: dict[str, Document],
                       graph: nx.Graph) -> List[List[Document]]:
